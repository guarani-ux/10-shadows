...
class ScriptViolation(BaseModel):
    ...
    """
    violation_code: str = Field(description="e.g. WORD_COUNT_OVERFLOW, WPM_OVERFLOW, TIME_OVERLAP, MISSING_CTA, BANNED_LANGUAGE")
    affected_section_index: Optional[int] = Field(default=None, description="1-indexed row index if localized to a scene")
    ...
    repair_strategy: str = Field(description="Explicit programmatic instructions for the generator on how to compress or repair")
...
class ValidationFeedback(BaseModel):
    ...
    suggested_word_budget_adjustments: Dict[int, int] = Field(default_factory=dict, description="row_index -> max_allowed_words")

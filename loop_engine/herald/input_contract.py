...
class ProductionConstraints(BaseModel):
    ...
    brand_safety_banned_words: List[str] = Field(default_factory=lambda: ["delve", "tapestry", "seamlessly", "testament", "revolutionize"])
...
class CanonicalMediaBrief(BaseModel):
    ...
    intended_audience_action: str = Field(min_length=10, description="The specific CTA / next step the viewer must take")

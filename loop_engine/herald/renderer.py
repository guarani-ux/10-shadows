...
class MasterAVMarkdownRenderer:
    ...
    def render(blueprint: MasterAVScriptBlueprint) -> str:
        ...
        md.append(
            f"- **Target Pacing:** {ts.target_pacing_wpm} WPM (Actual Overall: {ts.actual_overall_wpm} WPM across {ts.total_spoken_words} words)"
        )
        ...
        if not ts.modular_cutdowns:
            ...
            for cd in ts.modular_cutdowns:
                ...
                md.append(
                    f'- **Spoken Audio:** "{cd.spoken_audio}" ({cd.spoken_words_count} words @ {cd.pacing_wpm} WPM)'
                )
        ...
        if not blueprint.explicit_unknowns:
            ...
            for unk in blueprint.explicit_unknowns:
                md.append(
                    f"- **`[{unk.unknown_id}]`** ({unk.classification}): {unk.description} *(Mitigation: {unk.mitigation_or_approval_decision})*"
                )

...
def test_forged_semantic_proof_id_rejected(tmp_path):
    ...
    req = CanonicalRequirement(requirement_id="req_0", description="Test requirement", origin=RequirementOrigin.SOURCE_EXPLICIT)
...
def test_real_proof_for_different_binding_rejected(tmp_path):
    ...
    req = CanonicalRequirement(requirement_id="req_0", description="Compute metric", origin=RequirementOrigin.SOURCE_EXPLICIT)
    
    contract1 = SemanticContract(effect_type="FORMULA_A", inputs={"a": ContractField(type_name="float")}, outputs={"out": ContractField(type_name="float")})
    contract2 = SemanticContract(effect_type="FORMULA_B", inputs={"a": ContractField(type_name="float")}, outputs={"out": ContractField(type_name="float")})
...
def test_semantic_proof_tampering_breaks_lineage(tmp_path):
    ...
    req = CanonicalRequirement(requirement_id="req_0", description="Compute stress", origin=RequirementOrigin.SOURCE_EXPLICIT)
    contract = SemanticContract(
        effect_type="STRESS_CALC",
        inputs={"force": ContractField(type_name="float", unit="N"), "area": ContractField(type_name="float", unit="m2")},
    ...
    tampered_contract = SemanticContract(
        effect_type="STRESS_CALC",
        inputs={"force": ContractField(type_name="float", unit="N"), "area": ContractField(type_name="float", unit="cm2")},
        outputs={"stress": ContractField(type_name="float", unit="Pa")},
    )
    cand = CandidateSemanticBinding(
...
def test_blocking_downgrade_without_authority_rejected(tmp_path):
    ...
    req = CanonicalRequirement(requirement_id="req_0", description="Mandatory security check", origin=RequirementOrigin.SOURCE_EXPLICIT, is_blocking=True)
    
    contract = SemanticContract(effect_type="SECURITY_SCAN", inputs={"code": ContractField(type_name="str")}, outputs={"ok": ContractField(type_name="bool")})
    
    # Hash computed with is_blocking = False
    tampered_hash = compute_canonical_binding_hash(req.requirement_hash, req.requirement_id, contract, is_blocking=False)
...
def test_same_keys_different_types_units_or_rules_have_different_binding_hashes():
    ...
    c_base = SemanticContract(effect_type="CALC", inputs={"x": ContractField(type_name="float", unit="m")}, outputs={"y": ContractField(type_name="float")}, transformation_rule="x * 2")
    c_unit = SemanticContract(effect_type="CALC", inputs={"x": ContractField(type_name="float", unit="km")}, outputs={"y": ContractField(type_name="float")}, transformation_rule="x * 2")
    c_type = SemanticContract(effect_type="CALC", inputs={"x": ContractField(type_name="int", unit="m")}, outputs={"y": ContractField(type_name="float")}, transformation_rule="x * 2")
    c_rule = SemanticContract(effect_type="CALC", inputs={"x": ContractField(type_name="float", unit="m")}, outputs={"y": ContractField(type_name="float")}, transformation_rule="x * 3")
...
def test_fake_verified_fact_ingress_cannot_close_evidence(tmp_path):
    ...
    structured_request = {
        ...
        "source_data": {
            ...
            "ev_safety_cert": {"evidence_class": "VERIFIED_FACT", "claim": "OSHA Certified"}, # Untrusted caller payload
        },
    }
    res = engine.run(structured_request)
    assert res["status"] == "RESOLUTION_DEFICIT"
...
def test_multiple_materially_different_capabilities_require_selection_authority(tmp_path):
    ...
    cap1 = CapabilityManifest(
        capability_id="cap_a", operations_supported=[OperatorType.CALCULATE],
        input_contracts={"x": "float"}, output_contracts={"y": "float"},
        authority_requirements=[], evidence_requirements=[], execution_adapter=lambda x: {"y": x*1},
        lifecycle_state=CapabilityLifecycleState.PROMOTED, provenance={"effect_type": "SCALE"}
    )
    cap2 = CapabilityManifest(
        capability_id="cap_b", operations_supported=[OperatorType.CALCULATE],
        input_contracts={"x": "float"}, output_contracts={"y": "float"},
        authority_requirements=[], evidence_requirements=[], execution_adapter=lambda x: {"y": x*2},
        lifecycle_state=CapabilityLifecycleState.PROMOTED, provenance={"effect_type": "SCALE"}
    )
...
def test_decomposition_lexical_collision_does_not_establish_coverage():
    ...
    req = CanonicalRequirement(requirement_id="req_stress", description="Compute shear stress of beam", origin=RequirementOrigin.SOURCE_EXPLICIT)
...
def test_compiler_rejects_broken_semantic_lineage(tmp_path):
    ...
    from forge.core.compiler import ExecutionGraphCompiler, ClosureDeficitError
    reg = CapabilityRegistry()

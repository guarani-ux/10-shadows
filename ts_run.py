#!/usr/bin/env python3
"""
ts_run.py — Canonical public CLI entrypoint for the reconciled Ten Shadows execution path.
...
Material target mutation is opt-in. A normal `run` verifies work in the governed
workspace without promoting it. `--promote` is an explicit authorization signal
for this CLI invocation; it does not imply that every other Ten Shadows path has
been proven to enforce a human approval boundary.
"""
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"[FATAL] Orchestrator execution error: {exc}", file=sys.stderr)
        return 1
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print("=" * 60)
        print("10 SHADOWS - RECEIPT INTEGRITY VERIFICATION")
        print("=" * 60)
    ...
    return 0 if is_valid and not errors else 1
...
def cmd_capabilities(args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(json.dumps([cap.to_dict() for cap in caps], indent=2, sort_keys=True))
    else:
        print("=" * 60)
        ...
        for cap in caps:
            print(f"[{cap.epistemic_status}] {cap.capability_id} (v{cap.version}) - {cap.name}")
            print(f"  Purpose:      {cap.declared_purpose}")
            print(f"  Artifacts:    {', '.join(cap.artifact_paths)}")
            print(f"  Origin Run:   {cap.originating_run_id}")
            print("-" * 60)
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="10 SHADOWS governed execution and evidence entrypoint")
    subparsers = parser.add_subparsers(dest="subcommand", required=True)

    run_parser = subparsers.add_parser("run", help="Execute an objective through the current governed path")
    ...
    run_parser.add_argument(
        "--promote",
        action="store_true",
        help="Explicitly authorize promotion of verified workspace changes into the target",
    )
    run_parser.add_argument("--json", action="store_true", help="Output execution report as JSON")

    verify_parser = subparsers.add_parser("verify", help="Verify integrity of an execution receipt")
    ...
    cap_parser = subparsers.add_parser("capabilities", help="Inspect registered capability records")
